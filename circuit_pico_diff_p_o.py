...
class DifferentialPressureLogger():
    ...
    def timestamp(self) -> None:
        ...
        self.thermal_printer.printer.print("{}-{}-{}T{}:{}:{}".format(t.tm_year, t.tm_mon, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec))
    ...
    while True:
        ...
        if past_time <= time.time():
            if abs(delta) >= logger.threshold:
                ...
                logger.buzzer.pi()
                logger.timestamp()
                logger.thermal_printer.printer.print(str("diff_P:" + str(round(logger.ma_p, 4)) + ", delta:" + str(round(delta, 4))))
